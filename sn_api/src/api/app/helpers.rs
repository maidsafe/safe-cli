...
// TEMP
pub fn temp_pick_first_leaf<T>(
    leaves: std::collections::BTreeSet<(crate::register::EntryHash, T)>,
) -> Result<(crate::register::EntryHash, T)> {
    leaves
        .into_iter()
        .next()
        .ok_or(Error::ContentNotFound("No leaf in Register".to_string()))
}
