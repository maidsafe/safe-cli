...
use file_system::{file_system_dir_walk, normalise_path_separator};
...
impl Safe {
    ...
    pub async fn files_container_create(
        ...
        follow_links: bool,
        dry_run: bool,
    ) -> Result<(XorUrl, ProcessedFiles, FilesMap, EntryHash)> {
        ...
        /*
                ...
                Ok((xorurl, processed_files, files_map, hash))
        */
        unimplemented!()
    }
    /*
            ...
            pub async fn files_container_get(&mut self, url: &str) -> Result<(u64, FilesMap)> {
                ...
                self.fetch_files_container(&safeurl).await
            }
    */
    ...
    pub(crate) async fn fetch_files_container(
        &mut self,
        safeurl: &SafeUrl,
    ) -> Result<(EntryHash, FilesMap)> {
        // Check if the URL specifies a specific hash of the content or simply the current/latest available
        match self.fetch_register_value(safeurl).await {
            Ok(data) => {
                // FIXME: what if we get several values
                let (hash, serialised_files_map) = super::helpers::temp_pick_first_leaf(data)?;

                debug!("Files map retrieved with hash '{}'...", encode(hash));
                ...
                let files_map_xorurl = SafeUrl::from_url(
                    &String::from_utf8(serialised_files_map.clone()).map_err(|err| {
            ...
            Err(Error::EmptyContent(_)) => {
                warn!("FilesContainer found at \"{:?}\" was empty", safeurl);
                Ok((EntryHash::default(), FilesMap::default()))
            }
            Err(Error::ContentNotFound(_)) => Err(Error::ContentNotFound(
                ERROR_MSG_NO_FILES_CONTAINER_FOUND.to_string(),
            )),
            Err(Error::HashNotFound(_)) => Err(Error::HashNotFound(format!(
                "Hash '{}' is invalid for FilesContainer found at \"{}\"",
                safeurl
                    .content_hash()
                    .map_or("".to_string(), |hash| encode(hash)),
                safeurl,
            ))),
    ...
    /*
        ...
        pub async fn files_container_sync(
            ...
            if update_nrs && safeurl.content_type() != SafeContentType::NrsMapContainer {
                return Err(Error::InvalidInput(
                    "'update-nrs' is not allowed since the URL provided is not an NRS URL".to_string(),
                ));
            }

            let (mut safeurl, _) = self.parse_and_resolve_url(url).await?;

            // If the FilesContainer URL was resolved from an NRS name we need to remove
            // the content hash from it so we can fetch current/latest version of it for sync-ing
            safeurl.set_content_hash(None);

            let (current_hash, current_files_map): (EntryHash, FilesMap) =
                self.fetch_files_container(&safeurl).await?;
            ...
            let dest_path = Some(safeurl.path());
            ...
            Ok((new_hash, processed_files, new_files_map))
        }
        ...
        pub async fn files_container_add(
            ...
            let (safeurl, current_version, current_files_map) =
                validate_files_add_params(self, source_file, url, update_nrs).await?;

            let dest_path = safeurl.path();
        ...
        pub async fn files_container_add_from_raw(
            ...
            update_nrs: bool,
            dry_run: bool,
        ) -> Result<(EntryHash, ProcessedFiles, FilesMap)> {
            let (safeurl, current_hash, current_files_map) =
        ...
        pub async fn files_container_remove_path(
            ...
            // If the FilesContainer URL was resolved from an NRS name we need to remove
            // the content hash from it so we can fetch just current value of it
            safeurl.set_content_hash(None);

            let (hash, files_map): (EntryHash, FilesMap) =
                self.fetch_files_container(&safeurl).await?;
        ...
        // Private helper function to write a new version of the FilesMap to the Files Container
        // If flagged with `update_nrs`, it will also update the link in the corresponding NRS Map Container
        #[allow(clippy::too_many_arguments)]
        async fn append_version_to_files_container(
            &mut self,
            success_count: u64,
            current_hash: EntryHash,
            ...
            let version = if success_count == 0 {
                ...
                if update_nrs {
                    ...
                    safeurl.set_content_hash(Some(new_hash));
                    let new_link_for_nrs = safeurl.to_string();
                    let _ = self
    ...
    */
    ...
    pub async fn files_get_public_blob(&mut self, url: &str, range: Range) -> Result<Vec<u8>> {
        // TODO: do we want ownership from other PKs yet?
        let (safeurl, _) = self.parse_and_resolve_url(url).await?;
        self.fetch_public_blob(&safeurl, range).await
    }
    ...
    pub(crate) async fn fetch_public_blob(
        ...
        self.safe_client
            .get_public_blob(safeurl.xorname(), range)
            .await
    }
    /*
...
async fn validate_files_add_params(
    ...
    let (mut safeurl, _) = safe.parse_and_resolve_url(url).await?;

    // If the FilesContainer URL was resolved from an NRS name we need to remove
    // the content hash from it so we can fetch current/latest version of it for sync-ing
    safeurl.set_content_hash(None);

    let (hash, current_files_map): (EntryHash, FilesMap) = safe.fetch_files_container(&safeurl).await?;

    let dest_path = safeurl.path().to_string();

    // Let's act according to if it's a local file path or a safe:// location
    if source_file.starts_with("safe://") {
        let source_safeurl = Safe::parse_url(source_file)?;
        if source_safeurl.data_type() != SafeDataType::PublicBlob {
            return Err(Error::InvalidInput(format!(
                "The source URL should target a file ('{}'), but the URL provided targets a '{}'",
                SafeDataType::PublicBlob,
                source_safeurl.content_type()
            )));
        }
    ...
    Ok((safeurl, current_version, current_files_map))
}
*/
...
async fn files_map_add_link(
    ...
    match SafeUrl::from_url(file_link) {
        ...
        Ok(safeurl) => {
            ...
            let file_type = match safeurl.content_type() {
...
mod tests {
    ...
    /*
        ...
        async fn test_files_container_sync_update_nrs_unversioned_link() -> Result<()> {
            ...
            safeurl.set_content_hash(None);
            let unversioned_link = safeurl.to_string();
            match safe
        ...
        async fn test_files_container_sync_update_nrs_versioned_link() -> Result<()> {
            ...
            let (nrs_xorurl, _, _) = safe
                .nrs_map_container_create(&nrsurl, &safeurl.to_string(), false, true, false)
                .await?;
            ...
            assert_eq!(new_link.to_string(), safeurl.to_string());
        ...
        async fn test_files_container_sync_target_path_without_trailing_slash() -> Result<()> {
            ...
            assert_eq!(files_map.len(), TESTDATA_PUT_FILEITEM_COUNT);
            let mut safeurl = SafeUrl::from_url(&xorurl)?;
            safeurl.set_path("path/when/sync");
            let (version, new_processed_files, new_files_map) = safe
                .files_container_sync(
                    "../testdata/subfolder",
                    &safeurl.to_string(),
        ...
        async fn test_files_container_version() -> Result<()> {
            ...
            let (version, _) = retry_loop_for_pattern!(safe
                .files_container_get(&safeurl.to_string()), Ok((version, _)) if *version == 1)?;
            assert_eq!(version, 1);
        ...
        async fn test_files_container_get_with_version() -> Result<()> {
            ...
            let (version, v0_files_map) = safe
                .files_container_get(&safeurl.to_string())
                .await?;
            ...
            match safe.files_container_get(&safeurl.to_string()).await {
                Ok(_) => Err(anyhow!(
                    "Unexpectedly retrieved verion 2 of container".to_string(),
                )),
                Err(Error::HashNotFound(msg)) => {
                    assert_eq!(
                        msg,
                        format!(
