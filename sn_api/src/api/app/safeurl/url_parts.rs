...
use crate::{api::app::register::EntryHash, Error, Result};
...
pub(crate) struct SafeUrlParts {
    ...
    pub content_hash: Option<EntryHash>,
}
...
impl SafeUrlParts {
    ...
    pub fn parse(url: &str, ignore_labels_size: bool) -> Result<Self> {
        ...
        // ...and content hash from the Url fragment
        let content_hash = if let Some(fragment) = parsing_url.fragment() {
            let hash_bytes = hex_decode(fragment).map_err(|err| {
                Error::InvalidXorUrl(format!(
                    "Failed to decode the content hash '{}': {}",
                    fragment, err
                ))
            })?;

            let hash = hash_bytes.try_into().map_err(|v: Vec<_>| {
                Error::InvalidXorUrl(format!(
                    "Expected a content hash of length {} but it was {}",
                    size_of::<EntryHash>(),
                    v.len()
                ))
            })?;
        ...
        debug!(
            "Parsed url: scheme: {}, public_name: {}, top_name: {}, sub_names: {}, sub_names_vec: {:?}, path: {}, query_string: {}, content_hash: {:?}",
            scheme,
            public_name,
            top_name,
            sub_names,
            ...
            path,
            query_string,
            content_hash.map(|hash| Some(hex_encode(hash))),
        );
